...
def main():
    ...
    dataset = Planetoid(
        ...
        name='PubMed', transform=NormalizeFeatures()
    )
    # len(dataset[0])
    ...
    for epoch in range(1, 101):
        ...
        loss = torch.nn.CrossEntropyLoss()(pred[data_.train_mask], data_.y[data_.train_mask])
        loss.backward()
        optimizer.step()

        correct = pred[data_.train_mask].argmax(dim=1) == data_.y[data_.train_mask]
